...
public class DiscoveryServiceImpl implements DiscoveryService {
			...
			while (true) {
				...
				try {
					...
					if (ctrl.getEntityId() != DiscoveryConfiguration.DISCOVERY_CONFIG.getNATIONAL_CONTROLLER()
						...
						neighbor.setRemoteInterfaceIp(msgPacket.getAddress().toString().substring(1));
						neighbor.setLocalInterfaceIp(findLocalIp(msgPacket.getAddress()).toString().substring(1));
	...
	private InetAddress findLocalIp(InetAddress remoteIp) {
		for (Router router : DiscoveryConfiguration.DISCOVERY_CONFIG.getNetworkElements()) {
			InetAddress localIp = vtyRouterDAO.getIpMrouteSource(router, DiscoveryConfiguration.DISCOVERY_CONFIG.getMULTICAST_GROUP().toString().substring(1), remoteIp);
			if (localIp == null) {
				continue;
			}
			return localIp;
